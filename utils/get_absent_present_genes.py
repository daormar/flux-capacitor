...
def main(argv):
    ...
    try:
        ...
        print >> sys.stderr, "get_absent_present_genes -d <string> -p <string> [-c <int>]"
        sys.exit(2)
    for opt, arg in opts:
        if opt == '-h':
            print "get_absent_present_genes -d <string> -p <string> [-n <int>]"
            print ""
            print "-d <string> :    file with eset gene ids to entrez ids"
            print "-p <string> :    file with data generated by panp R library"
            print "-c <int>    :    get abs/pres data for <int>'th column"
            sys.exit()
        elif opt in ("-d", "--dictf"):
            dictf = arg
            d_given=True
